...
C				mp should be defined here the same as in fread
	parameter (mp=200)
     ...
     &       TRITON(8),COEF(mp),BPHASE(NKMAX,MSP),FORMIN(2*MAXM),HCM
      ...
      LOGICAL TKNRM,PWF,TKMAT,REN(MLOC),FUSED(1000),LIAP
...
!     &         KBPOT,KRPOT,E,ISC,IPC,NFL,NAM,AMPL
	KN1=0;IC1=0;IC2=0;IN=0;BE=0.0;IA=0
	read(KI,nml=overlap)
...
C8858       FORMAT('0File ',I3,' needs RL =',I3,' ',A9,' numbers')
...
23    FORMAT(/'0UNUSABLE COMBINATION OF CHANNEL AND/OR QUANTUM NUMBERS :
     & '   /,'0',2I4,':',3I3,1X,'?'/)
            GO TO 890
            ENDIF
...
27    FORMAT('0BOUND PARTIAL WAVE AT',I3,' IS',16I4,', ASYMP BE =',F8.4)
28    CONTINUE
29    CONTINUE
30    CONTINUE
      IF(KN.GT.KN2) THEN
...
31         FORMAT(/' NO ROOM FOR',I4,' MORE CHANNELS: INCREASE KN2')
      ...
      QNF(17,KN1:KN2) = M
         IF(M.EQ.0) THEN
          WRITE(KO,*) '0 NO SUITABLE CHANNELS FOUND ????  INPUT DATA WER
     XE:'
...
C           are there only because of definition of M(Ek) matrix element
         ...
         IF(KP.EQ.KBPOT) THEN
            IF(ISC.EQ.0 .OR. POTK.EQ.0 .OR. .NOT.EIGEN) THEN
                I = 1
...
155          format('# KN1,E,l,j,2j+1,N =',i4,f10.5,i4,f5.1,2i4)
        ...
        DO 60 I=1,MINT-1
         R = MAX(I-1.,0.01)*HDN
	   if(rlb) then
           IF(NFL.LE.0) FORMR(I) = PSI(I,IC) / R
	    if(mod(IPC,2)>0.and.final) write(58,*) R,FORMR(I)*R
           WWW = FORMR(I)
	   else
           IF(NFL.LE.0) FRMC(I) = PSIC(I,IC) / R
	    if(mod(IPC,2)>0.and.final) write(58,*) R,FRMC(I)*R
...
156		format(f8.3,2g15.5)
     ...
     x             .or.I.eq.MINT-1)) THEN
	       ...
	       if(final) then
		call openif(46)
             	write(46,59) R,ANC,WWW,WL(L+1),WLD(L+1)/WL(L+1),IE
		written(46) = .true.
59       	format(f9.3,4g15.6,i4)
	       endif
         ENDIF
60    CONTINUE
...
70     if(     cxwf) FORMC(I,KM,1) = TC
       ...
       FRMC(:) = 0.0
C
      IF(NFL.LE.0 .AND. KIND.NE.4) THEN
      DO 86 JF=1,NF
      DO 86 IP=1,M
      IF(KRPOT.GT.0) THEN
...
80       CONTINUE
86    CONTINUE
       ELSE IF(NFL.LE.0 .AND. KIND.EQ.4) THEN
         DO 87 I=2,MINT-1
         R = (I-1)*HDN
...
858   FORMAT('0',I4,':',4I3,2X,2A1,i2,I3,F4.1,I3,F4.1,I4,I4,I4,F9.4,2I3,
     &  I3,F7.4,1X,A3,F7.4,';',F3.0,F6.3,F7.4,F7.4,F7.3,F7.1,F7.1,F8.4)
8581  FORMAT('0',I4,':',4I3,2X,2A1,i2,I3,F4.1,I3,F4.1,I4,I4,I4,F9.4,2I3,
...
861      QNF(I,KN) = 0
     ...
     &                         WD0,WD
859   FORMAT('0Overall rms norm & radius =',2F8.4,', & Overall D0 & D ='
     &,2F9.2)
      if(.not.EIGEN.and.NFL<=0) deallocate(PSIC)
...
890     if(allocated(PSI)) deallocate (PSI)
        ...
        if(ISC<0) THEN ! renormalise all varied potentials permanently!
         DO JF=1,NF
         if(REN(JF)) then
