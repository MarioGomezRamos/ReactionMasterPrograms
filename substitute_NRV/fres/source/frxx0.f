...
!@     X   gailitis,gailacc,
	...
	ko3 = 301
        ...
        write(koe,1002) 
 1002 	format(' FRESCO - FRES 2.9: Coupled Reaction Channels')
	uu = .false.
...
!1031 	format(f4.0,i4,f8.4,l2,1x,a1,1x,a1,i2,10i4)
 ...
 1040 FORMAT(//' Range of total J is',F8.1,' <= J <=',F8.1,
     ...
     X  'Relativistic kinematics = ',A1,', Both/Far/Near Analyses =',I3)
      IF(RELA.ne.' ')
     X    WRITE(*,*) 'RELATIVISTIC KINEMATICS NOT IMPLEMENTED!!'
      NJ = 0
      DO K=7,1,-1
       IF(JJUMP(K,1)/=0) then
	NJ = K
...
10751  format('   READ FIXED CHANNEL WFS from file #',i4)
     ...
     x			ebeta,weak
 1076    format(2i4,i1,l1,I2,i2,f6.4,3f8.4)
	endif
...
!why?	   md = mrm - 6
     ...
     x                   pralpha,pcon,meigs,rmatr,ebeta,weak
...
!         write(0,*) 'icto,icfrom=',icto,icfrom
  118    if ( ios < 0 ) then
            nlcouplingend=1
          endif 
