...
C    INCOMING ENERGIES
C    -----------------
      REAL*8 K(MXP,MXX),ETA(MXP,MXX),ETOTAL,ECMC,ENLAB,EOFF,
...
C
      ...
      ETOTAL = ENLAB * MASS(3-LIN,LAB)/(MASS(2,LAB)+MASS(1,LAB)) + EOFF
...
C              CLOSED CHANNELS:
	...
	if(DERIV) then
           DO  L1=1,MAL1
              CHL(L1,IT,2) = dcmplx(0d0,CFG(L1,2) * T)
	   ENDDO
	else
         R = (MD-1)*HP(IC)
        CALL WHIT(ETA(IC,IA),R,K(IC,IA),ECMC,MAL1,CFG,CFG(1,2),IE)
...
C    ---------------------------------------------------
      ...
      DO 290 CP=1,NCP
            ...
            IF(.NOT.LOCAL(CP).AND.ITER==0.and.IBLOCK>=0) GO TO 290
         IC1 = ICTO(CP)
         IC2 = ICFROM(CP)
...
C      IF(I.GT.0) STOP 'COULOMB MONOPOLES'
       IF(I.GT.0) write(6,*) ' BAD Coulomb deficiences! ',I
...
!657   FORMAT(I4,2F8.4,F8.1,I3,2f12.6,2f8.3)
      DO 1690 C2=1,NCH     ! look for at most NCH channel wfs !
        IF(INITWF.GT.0)
